...
async function signInWithGoogle() {
  ...
  try {
    ...
    // На телефоне popup часто не работает — используем redirect
...
// ---------- START ----------
// Обработка возврата с Google на мобильном (redirect)
auth.getRedirectResult().catch(err => {
  console.error('redirect error', err);
  if (err && err.code) {
    toast('Ошибка Google: ' + err.code);
    const el = $('authError');
    if (el) { el.textContent = err.message || err.code; show(el); }
  }
});
...
auth.onAuthStateChanged(async (user) => {
  if (user) {
    ...
    try {
      ...
      toast('Ошибка профиля: ' + msg);
      const el = $('authError');
      if (el) {
        el.textContent = 'Не удалось создать профиль: ' + msg + '. Проверь Firestore Rules.';
        show(el);
      }
      // не выкидываем сразу — чтобы увидеть ошибку
    }
  } else {
    currentUser = null;
    profile = null;
